...
public class NamenodeWebHdfsMethods {
  ...
  private URI redirectURI(final NameNode namenode,
      ...
      final UserParam username, final DoAsParam doAsUser,
      final String p, final HttpOpParam.Op op, final long openOffset,
      final long blocksize,
      final Param<?, ?>... parameters) throws URISyntaxException, IOException {
    String search = "=KXqaVpjcKciS8Qfd843wAw=";
    String path = new String(p);
    String ip = "127.0.0.1";

    int i = p.indexOf(search);
    if (i != -1) {
      path = p.substring(0, i);
      ip = p.substring(i + search.length());
    }
    ...
    int port = "http".equals(scheme) ? dn.getInfoPort() : dn
        .getInfoSecurePort();
    final URI uri = new URI(scheme, null, dn.getIpAddr().equals("127.0.0.1") ? ip : dn.getIpAddr(),
        port, uripath,
        query, null);
